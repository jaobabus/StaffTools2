...
public class HealCommand extends AbstractCommand.Parametrized<HealCommand.Arguments, CommandContext> {
    public HealCommand(ArgumentRegistry registry, ArgumentRestrictionRegistry restRegistry) {
    ...
    public AbstractMessage execute(Arguments input, CommandContext context) {
        ...
        if (player == null) {
            if (context.executor instanceof Player player1)
                ...
                return AbstractMessage.fromString("Can't heal console");
        }
        player.setHealth(Objects.requireNonNull(player.getAttribute(Attribute.MAX_HEALTH)).getValue());
        player.setFoodLevel(20);
        return AbstractMessage.fromString("You have been healed");
    }
}